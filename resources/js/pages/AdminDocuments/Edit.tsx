// AdminDocuments/Edit.tsx
import { type BreadcrumbItem } from '@/types';
import AppLayout from '@/layouts/app-layout';
import React, { useState } from 'react';
import { Head, router, Link } from '@inertiajs/react';
import { Document, Station } from '@/types';
import {
  Save,
  ArrowLeft,
  FileText,
...
interface AdminDocumentsEditProps {
  document: Document;
  stations: Station[];
}
...
const AdminDocumentsEdit: React.FC<AdminDocumentsEditProps> = ({ document, stations }) => {
  const [title, setTitle] = useState(document.title);
  const [category, setCategory] = useState(document.category);
  const [description, setDescription] = useState(document.description || '');
  const [stationId, setStationId] = useState<number | null>(document.station_id);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    router.post(`/admin/documents/${document.id}`, formData, {
        ...
        <div className="sm:flex sm:items-center sm:justify-between mb-8">
          <div>
            <div className="flex items-center">
              ...
              <h1 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Edit Document</h1>
            </div>
            <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
              Update document information and files.
            </p>
          </div>
            ...
            <div className="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
              ...
              <div className="sm:col-span-6">
                <label htmlFor="pdf_file" className="block text-sm font-medium text-gray-700 dark:text-gray-300 flex items-center">
                  <Upload className="h-4 w-4 mr-1" /> Update PDF File (Optional)
                </label>
                ...
                <div
                  className={`mt-3 flex justify-center px-6 pt-5 pb-6 border-2 border-dashed rounded-md ${
                    dragActive
                  ...
                  <div className="space-y-1 text-center">
                    ...
                    {pdfFile && (
                      <p className="text-sm text-indigo-600 dark:text-indigo-400 font-medium">
                        Selected new file: {pdfFile.name}
                      </p>
                    )}
                  </div>
                </div>
                <div className="mt-2 p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-md">
                  <div className="flex">
                    <div className="flex-shrink-0">
                      <AlertTriangle className="h-5 w-5 text-yellow-400" aria-hidden="true" />
                    </div>
                    <div className="ml-3">
                      <h3 className="text-sm font-medium text-yellow-800 dark:text-yellow-200">Note</h3>
                      <div className="mt-2 text-sm text-yellow-700 dark:text-yellow-300">
                        <p>
                          Only upload a new PDF if you want to replace the existing file. Leave this empty to keep the current file.
                        </p>
                      </div>
                    </div>
                  </div>
